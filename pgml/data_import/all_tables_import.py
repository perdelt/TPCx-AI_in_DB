...
def custom_csv_parser(file_path):
    # Try to detect encoding - start with common European encodings
    encodings = ['latin-1', 'windows-1252', 'utf-8']
    detected_encoding = None
    
    for encoding in encodings:
        try:
            with open(file_path, 'r', encoding=encoding) as test_file:
                # Try to read a significant portion to confirm encoding works
                test_file.read(20000)
    ...
    if not detected_encoding:
        detected_encoding = 'utf-8'  # Default fallback with replace errors
    ...
    with tempfile.NamedTemporaryFile(mode='w+', delete=False, newline='', encoding='utf-8') as temp_file:
        with open(file_path, 'r', encoding=detected_encoding, errors='ignore') as infile:
        ...
        return temp_file.name  # This file is now UTF-8 encoded
...
#                 for line in file:
#                     copy.write(line.encode('utf-8', errors='ignore').decode('utf-8'))

#         finally:
#             pass
def import_csv_to_table(cursor, file_path, headers, table_name, schema='public'):
    # Ensure the database connection's client encoding is set to UTF-8
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        column_names = ', '.join([f'"{name.lower()}"' for name in headers])
...
if __name__ == '__main__':
    ...
    data_path = os.getenv('DATA_PATH') + os.getenv('TPCxAI_SCALE_FACTOR')
    print(f'Looking for data at: {data_path} \n with scale factor: {os.getenv("TPCxAI_SCALE_FACTOR")}')

    # Path to generate_tables.sql file
    generate_tables_sql = os.getenv('generate_tables_sql')
    create_indexes_sql = os.getenv('create_indexes_sql')
    
    # Database connection parameters
    db_name = os.getenv('DATABASE') if os.getenv('DATABASE') else "postgres"
    db_user = os.getenv('DB_USER') if os.getenv('USER') else "postgres"
